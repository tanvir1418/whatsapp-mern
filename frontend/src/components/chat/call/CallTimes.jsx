...
const CallTimes = ({ totalSecInCall, setTotalSecInCall, callAccepted }) => {
    useEffect(() => {
        const setSecInCall = () => {
    ...
    return (
        <div className={`text-dark_text_2 ${totalSecInCall !== 0 ? "block" : "hidden"}`}>
            ...
            <span>{parseInt(totalSecInCall / 60).toString().length < 2 ? "0" + parseInt(totalSecInCall / 60) : parseInt(totalSecInCall / 60)}</span>
            <span>:</span>
